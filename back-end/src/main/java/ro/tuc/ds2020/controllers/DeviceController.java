...
public class DeviceController {
    ...
    @GetMapping(value="/forclient/{id}")
    public ResponseEntity<List<DeviceDTO>> getDevicesForUser(@PathVariable("id") UUID id_user) {
        List<DeviceDTO> dtos = deviceService.findDevicesByUser(id_user);
        for (DeviceDTO dto : dtos) {
            Link deviceLink = linkTo(methodOn(DeviceController.class)
                    .getDevice(dto.getId())).withRel("deviceDetails");
            dto.add(deviceLink);
